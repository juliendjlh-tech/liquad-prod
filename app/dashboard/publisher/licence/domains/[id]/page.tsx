...
export default function DomainDetailPage() {
  ...
  // Sitemap import status state
  const [indexStatus, setIndexStatus] = useState<
    "pending" | "processing" | "completed" | "failed" | null
  >(null);
  const [indexTotalUrls, setIndexTotalUrls] = useState<number | null>(null);
  const [indexErrorMessage, setIndexErrorMessage] = useState<string | null>(null);
  const [indexLastRunAt, setIndexLastRunAt] = useState<string | null>(null);
  const [reindexing, setReindexing] = useState(false);

  // Fetch domain info (name + sitemap import status)
  const fetchDomainInfo = useCallback(async () => {
    ...
    if (res.ok) {
      ...
      setIndexStatus(data.index_status ?? null);
      setIndexTotalUrls(data.index_total_urls ?? null);
      setIndexErrorMessage(data.index_error_message ?? null);
      setIndexLastRunAt(data.index_last_run_at ?? null);
    }
  ...
  const executeDeleteDomain = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => router.push("/dashboard/publisher/licence/domains"), 1000);
      } else {
    ...
    <div>
      ...
      <div className="mb-6">
        <div className="flex gap-2 justify-end items-center">
          ...
          <Button
            size="sm"
            onClick={() => router.push(`/dashboard/publisher/licence/domains/${domainId}/import`)}
          >
            Index contents
          </Button>
          <Button
      ...
      {/* Sitemap import status section */}
      <DomainIndexingStatus
        status={indexStatus}
        totalUrls={indexTotalUrls}
        errorMessage={indexErrorMessage}
        lastRunAt={indexLastRunAt}
