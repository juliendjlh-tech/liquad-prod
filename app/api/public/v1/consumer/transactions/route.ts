...
/**
 * GET /api/public/v1/consumer/transactions
 *
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    let query = supabase
      ...
      .select(
        "id, role, amount_eur, content_url, publisher_workspace_id, created_at"
      )
    ...
    return NextResponse.json({
      items: results.map((t) => ({
        id: t.id,
        // `role` replaces the legacy `type` column (migration 042).
        // Expose as `type` for backward compat with existing consumers.
        type: t.role,
        amount_eur: Number(t.amount_eur),
        content_url: t.content_url,
        publisher_workspace_id: t.publisher_workspace_id,
