-- ============================================================================
-- Migration 028: Amazonbot IPs + preset descriptions backfill
-- ============================================================================
--
-- Fills two gaps left by migration 027:
--
-- 1. Amazonbot declared_ips. Migration 027 could not reach the Amazonbot
--    endpoint. The correct source embeds a JSON blob (field `ip_prefix`, all
--    /32 IPv4) at:
--      https://developer.amazon.com/amazonbot/searchbot-ip-addresses/
--    Fetched 2026-04-23, page creationTime 2025-11-21T23:40:00.471564Z,
--    512 unique IPv4 /32 prefixes, no IPv6.
--
-- 2. Preset descriptions. Migration 026 added `agents.description` but nothing
--    ever populated it. The 25 preset descriptions below are copied verbatim
--    from AI_BOT_PRESETS in lib/services/agent.service.ts.
--
-- IMPORTANT: No production data exists. Safe to run.
-- ============================================================================


-- ============================================================================
-- 1. Amazonbot declared_ips (512 prefixes, /32 IPv4)
-- ============================================================================

UPDATE public.agents
SET declared_ips = ARRAY[
  '100.24.190.202/32',
  '100.25.127.167/32',
  '100.25.153.201/32',
  '100.25.17.231/32',
  '100.25.30.152/32',
  '100.26.122.24/32',
  '100.27.85.243/32',
  '100.28.152.127/32',
  '100.29.157.182/32',
  '100.29.59.194/32',
  '100.48.0.143/32',
  '107.20.124.65/32',
  '107.20.132.237/32',
  '107.20.164.28/32',
  '107.20.195.185/32',
  '107.20.21.185/32',
  '107.21.62.197/32',
  '107.21.9.54/32',
  '107.22.145.162/32',
  '107.23.25.176/32',
  '13.216.102.104/32',
  '13.216.187.148/32',
  '13.216.196.168/32',
  '13.219.170.211/32',
  '13.219.54.187/32',
  '13.222.91.251/32',
  '13.223.109.92/32',
  '13.223.163.204/32',
  '13.223.222.73/32',
  '174.129.228.153/32',
  '18.204.69.251/32',
  '18.204.90.3/32',
  '18.205.13.0/32',
  '18.205.140.254/32',
  '18.205.180.59/32',
  '18.206.202.183/32',
  '18.206.50.226/32',
  '18.206.69.92/32',
  '18.208.43.113/32',
  '18.209.135.224/32',
  '18.209.142.234/32',
  '18.209.178.202/32',
  '18.210.115.131/32',
  '18.210.225.20/32',
  '18.213.133.4/32',
  '18.213.249.250/32',
  '18.214.124.101/32',
  '18.214.125.232/32',
  '18.214.206.100/32',
  '18.214.84.4/32',
  '18.215.135.45/32',
  '18.215.42.159/32',
  '18.215.99.115/32',
  '18.233.214.108/32',
  '18.233.235.66/32',
  '18.233.44.129/32',
  '18.235.234.69/32',
  '184.72.137.83/32',
  '184.73.154.145/32',
  '23.20.194.146/32',
  '23.21.115.175/32',
  '23.21.214.230/32',
  '23.21.254.26/32',
  '23.21.72.195/32',
  '23.22.245.96/32',
  '23.22.4.24/32',
  '23.23.169.169/32',
  '23.23.188.58/32',
  '23.23.201.125/32',
  '23.23.214.73/32',
  '23.23.97.185/32',
  '3.208.113.39/32',
  '3.209.71.33/32',
  '3.210.117.45/32',
  '3.210.238.203/32',
  '3.210.239.195/32',
  '3.211.100.186/32',
  '3.211.143.75/32',
  '3.212.138.251/32',
  '3.212.155.47/32',
  '3.213.253.252/32',
  '3.214.235.2/32',
  '3.214.252.9/32',
  '3.214.51.149/32',
  '3.215.155.189/32',
  '3.215.205.141/32',
  '3.217.161.64/32',
  '3.217.34.58/32',
  '3.218.37.5/32',
  '3.219.192.9/32',
  '3.219.6.124/32',
  '3.221.116.68/32',
  '3.221.167.25/32',
  '3.221.56.120/32',
  '3.221.61.12/32',
  '3.222.127.245/32',
  '3.222.19.80/32',
  '3.222.196.181/32',
  '3.223.100.234/32',
  '3.223.139.87/32',
  '3.223.242.201/32',
  '3.223.42.33/32',
  '3.224.123.189/32',
  '3.224.216.148/32',
  '3.224.45.108/32',
  '3.224.73.194/32',
  '3.225.111.192/32',
  '3.225.200.77/32',
  '3.226.139.230/32',
  '3.226.196.238/32',
  '3.226.3.62/32',
  '3.226.50.226/32',
  '3.227.142.177/32',
  '3.227.227.6/32',
  '3.228.151.92/32',
  '3.229.219.29/32',
  '3.229.67.114/32',
  '3.229.72.127/32',
  '3.230.253.54/32',
  '3.230.59.35/32',
  '3.231.178.162/32',
  '3.233.189.187/32',
  '3.234.13.59/32',
  '3.234.152.213/32',
  '3.234.28.73/32',
  '3.82.67.224/32',
  '3.82.90.168/32',
  '3.83.75.212/32',
  '3.85.141.163/32',
  '3.89.155.158/32',
  '3.89.17.222/32',
  '3.90.117.221/32',
  '3.90.92.205/32',
  '3.92.140.162/32',
  '3.94.185.94/32',
  '34.192.213.30/32',
  '34.192.224.176/32',
  '34.192.94.60/32',
  '34.193.176.214/32',
  '34.193.28.138/32',
  '34.194.196.150/32',
  '34.194.236.194/32',
  '34.194.81.161/32',
  '34.194.96.61/32',
  '34.195.194.181/32',
  '34.195.79.160/32',
  '34.195.90.205/32',
  '34.196.127.79/32',
  '34.196.221.51/32',
  '34.197.240.80/32',
  '34.197.246.201/32',
  '34.197.25.140/32',
  '34.197.27.62/32',
  '34.198.117.140/32',
  '34.198.223.196/32',
  '34.199.111.130/32',
  '34.199.2.135/32',
  '34.200.178.28/32',
  '34.200.190.72/32',
  '34.201.229.234/32',
  '34.201.89.57/32',
  '34.203.29.230/32',
  '34.203.33.121/32',
  '34.204.234.171/32',
  '34.204.31.92/32',
  '34.204.82.126/32',
  '34.206.13.250/32',
  '34.206.160.62/32',
  '34.206.189.108/32',
  '34.207.190.20/32',
  '34.207.31.244/32',
  '34.224.104.19/32',
  '34.224.135.86/32',
  '34.224.5.130/32',
  '34.225.204.15/32',
  '34.225.241.157/32',
  '34.226.171.86/32',
  '34.226.176.13/32',
  '34.227.210.84/32',
  '34.227.211.218/32',
  '34.228.0.97/32',
  '34.228.124.71/32',
  '34.229.55.190/32',
  '34.231.198.5/32',
  '34.232.233.214/32',
  '34.233.191.178/32',
  '34.234.177.179/32',
  '34.234.21.24/32',
  '34.234.221.209/32',
  '34.235.221.79/32',
  '34.236.163.131/32',
  '34.236.65.219/32',
  '34.237.37.16/32',
  '34.238.12.204/32',
  '34.238.132.4/32',
  '34.238.63.49/32',
  '34.238.66.168/32',
  '34.239.253.240/32',
  '34.239.87.171/32',
  '34.239.90.1/32',
  '35.168.147.87/32',
  '35.168.181.24/32',
  '35.168.48.89/32',
  '35.168.75.176/32',
  '35.169.1.17/32',
  '35.169.2.39/32',
  '35.169.74.227/32',
  '35.170.104.4/32',
  '35.170.123.50/32',
  '35.171.77.201/32',
  '35.172.188.13/32',
  '35.173.101.225/32',
  '35.174.28.56/32',
  '35.175.132.11/32',
  '44.193.34.148/32',
  '44.194.124.103/32',
  '44.194.165.181/32',
  '44.194.47.12/32',
  '44.195.217.176/32',
  '44.196.83.48/32',
  '44.197.254.236/32',
  '44.198.230.203/32',
  '44.198.66.156/32',
  '44.199.112.216/32',
  '44.199.145.199/32',
  '44.199.62.58/32',
  '44.205.208.84/32',
  '44.206.127.11/32',
  '44.206.217.175/32',
  '44.206.81.21/32',
  '44.207.64.241/32',
  '44.208.239.245/32',
  '44.209.121.79/32',
  '44.209.167.138/32',
  '44.210.169.22/32',
  '44.210.170.50/32',
  '44.210.179.109/32',
  '44.210.189.34/32',
  '44.210.225.188/32',
  '44.210.41.97/32',
  '44.211.6.49/32',
  '44.212.207.23/32',
  '44.213.228.74/32',
  '44.215.171.38/32',
  '44.215.74.155/32',
  '44.215.99.191/32',
  '44.216.177.139/32',
  '44.217.164.115/32',
  '44.217.204.19/32',
  '44.217.221.211/32',
  '44.217.237.69/32',
  '44.217.64.212/32',
  '44.218.56.203/32',
  '44.218.61.226/32',
  '44.219.225.200/32',
  '44.219.225.212/32',
  '44.221.129.95/32',
  '44.221.203.99/32',
  '44.221.241.220/32',
  '44.223.253.120/32',
  '50.17.88.23/32',
  '50.19.173.230/32',
  '52.0.207.54/32',
  '52.20.158.171/32',
  '52.20.27.34/32',
  '52.20.85.108/32',
  '52.200.125.80/32',
  '52.200.202.121/32',
  '52.201.26.95/32',
  '52.201.77.2/32',
  '52.202.160.12/32',
  '52.202.182.148/32',
  '52.202.30.225/32',
  '52.203.139.107/32',
  '52.203.235.174/32',
  '52.204.1.121/32',
  '52.205.16.194/32',
  '52.206.116.153/32',
  '52.206.129.89/32',
  '52.206.188.45/32',
  '52.206.29.234/32',
  '52.206.59.229/32',
  '52.207.6.18/32',
  '52.21.133.215/32',
  '52.21.93.114/32',
  '52.21.99.253/32',
  '52.22.52.60/32',
  '52.3.193.39/32',
  '52.3.251.73/32',
  '52.3.30.234/32',
  '52.4.189.224/32',
  '52.4.251.246/32',
  '52.44.115.134/32',
  '52.45.194.99/32',
  '52.45.83.81/32',
  '52.5.113.11/32',
  '52.5.12.245/32',
  '52.5.20.105/32',
  '52.54.130.150/32',
  '52.6.171.10/32',
  '52.6.216.48/32',
  '52.6.231.0/32',
  '52.6.24.217/32',
  '52.6.54.116/32',
  '52.7.126.210/32',
  '52.7.171.101/32',
  '52.7.225.39/32',
  '52.7.227.80/32',
  '52.7.249.12/32',
  '52.70.226.93/32',
  '52.70.37.172/32',
  '52.70.95.48/32',
  '52.71.16.223/32',
  '52.71.205.86/32',
  '52.71.243.5/32',
  '52.72.227.157/32',
  '52.73.35.133/32',
  '54.145.161.111/32',
  '54.145.34.70/32',
  '54.146.71.85/32',
  '54.147.229.64/32',
  '54.157.138.252/32',
  '54.157.208.183/32',
  '54.157.89.253/32',
  '54.158.133.188/32',
  '54.159.237.70/32',
  '54.161.157.185/32',
  '54.161.25.53/32',
  '54.162.54.187/32',
  '54.163.123.236/32',
  '54.163.22.228/32',
  '54.165.171.73/32',
  '54.165.202.67/32',
  '54.165.95.216/32',
  '54.166.24.13/32',
  '54.166.70.223/32',
  '54.167.133.42/32',
  '54.167.188.181/32',
  '54.167.5.166/32',
  '54.167.94.112/32',
  '54.172.176.34/32',
  '54.172.92.66/32',
  '54.173.160.86/32',
  '54.173.161.122/32',
  '54.174.161.242/32',
  '54.204.10.196/32',
  '54.204.159.166/32',
  '54.204.189.191/32',
  '54.205.137.236/32',
  '54.205.6.162/32',
  '54.205.7.250/32',
  '54.209.139.3/32',
  '54.209.33.199/32',
  '54.209.59.28/32',
  '54.210.16.158/32',
  '54.210.51.122/32',
  '54.224.159.224/32',
  '54.225.220.226/32',
  '54.226.54.148/32',
  '54.227.122.71/32',
  '54.227.95.204/32',
  '54.234.109.91/32',
  '54.234.227.173/32',
  '54.235.183.179/32',
  '54.235.187.99/32',
  '54.236.146.97/32',
  '54.236.251.98/32',
  '54.237.229.245/32',
  '54.242.155.65/32',
  '54.243.119.155/32',
  '54.243.128.188/32',
  '54.243.172.250/32',
  '54.243.185.76/32',
  '54.243.78.23/32',
  '54.81.126.112/32',
  '54.81.15.167/32',
  '54.81.2.245/32',
  '54.81.32.142/32',
  '54.82.2.214/32',
  '54.83.126.221/32',
  '54.83.172.18/32',
  '54.83.58.194/32',
  '54.84.160.219/32',
  '54.84.71.94/32',
  '54.85.38.148/32',
  '54.85.95.196/32',
  '54.87.255.90/32',
  '54.88.236.97/32',
  '54.89.16.186/32',
  '54.89.45.9/32',
  '54.89.69.250/32',
  '54.89.70.93/32',
  '54.90.18.3/32',
  '54.90.227.223/32',
  '54.90.28.43/32',
  '54.92.148.34/32',
  '72.44.45.175/32',
  '75.101.144.251/32',
  '98.80.83.102/32',
  '98.82.238.39/32',
  '98.82.61.199/32',
  '98.82.68.7/32',
  '98.83.194.21/32',
  '98.84.185.30/32',
  '98.84.242.117/32',
  '98.84.86.216/32',
  '98.85.153.239/32',
  '98.85.191.237/32',
  '98.86.101.20/32',
  '98.86.133.40/32',
  '98.86.217.46/32',
  '98.86.33.14/32',
  '98.86.38.110/32',
  '98.86.69.65/32',
  '98.87.203.235/32',
  '98.87.218.210/32',
  '98.88.114.29/32',
  '98.88.146.174/32',
  '98.88.186.79/32',
  '98.88.234.184/32',
  '98.88.240.41/32',
  '98.88.3.219/32',
  '98.88.4.160/32',
  '98.88.43.210/32',
  '98.88.67.17/32',
  '98.89.110.216/32',
  '98.89.176.131/32',
  '98.89.191.68/32',
  '98.89.198.186/32',
  '98.89.238.137/32',
  '98.89.254.223/32',
  '98.89.255.185/32',
  '98.89.74.239/32',
  '98.89.89.208/32',
  '98.90.1.201/32',
  '98.90.129.48/32',
  '98.90.132.132/32',
  '98.90.144.220/32',
  '98.90.155.195/32',
  '98.90.166.177/32',
  '98.90.171.206/32',
  '98.90.212.161/32',
  '98.90.53.3/32',
  '98.90.53.72/32',
  '98.90.93.167/32',
  '98.91.104.211/32',
  '98.91.120.100/32',
  '98.91.142.137/32',
  '98.91.147.104/32',
  '98.91.150.214/32',
  '98.91.160.54/32',
  '98.91.3.186/32',
  '98.91.81.246/32',
  '98.91.91.18/32',
  '98.94.126.135/32',
  '98.94.137.118/32',
  '98.94.149.8/32',
  '98.94.158.230/32',
  '98.94.164.245/32',
  '98.94.174.228/32',
  '98.94.174.49/32',
  '98.94.182.242/32',
  '98.94.188.12/32',
  '98.94.193.28/32',
  '98.94.196.171/32',
  '98.94.202.176/32',
  '98.94.203.213/32',
  '98.94.208.188/32',
  '98.94.209.71/32',
  '98.94.216.122/32',
  '98.94.225.199/32',
  '98.94.228.253/32',
  '98.94.237.222/32',
  '98.94.246.215/32',
  '98.94.252.84/32',
  '98.94.253.137/32',
  '98.95.1.231/32',
  '98.95.100.253/32',
  '98.95.104.207/32',
  '98.95.106.159/32',
  '98.95.110.94/32',
  '98.95.120.52/32',
  '98.95.126.63/32',
  '98.95.129.148/32',
  '98.95.130.0/32',
  '98.95.145.11/32',
  '98.95.148.56/32',
  '98.95.150.125/32',
  '98.95.156.127/32',
  '98.95.161.103/32',
  '98.95.161.76/32',
  '98.95.162.121/32',
  '98.95.174.52/32',
  '98.95.175.92/32',
  '98.95.20.69/32',
  '98.95.23.214/32',
  '98.95.23.228/32',
  '98.95.34.178/32',
  '98.95.37.200/32',
  '98.95.38.161/32',
  '98.95.39.39/32',
  '98.95.44.247/32',
  '98.95.49.54/32',
  '98.95.55.232/32',
  '98.95.56.240/32',
  '98.95.56.98/32',
  '98.95.62.182/32',
  '98.95.65.140/32',
  '98.95.7.71/32',
  '98.95.74.218/32',
  '98.95.89.44/32',
  '98.95.91.59/32'
]::text[]
WHERE name = 'Amazonbot';


-- ============================================================================
-- 2. Preset descriptions (25 presets)
--    Source: AI_BOT_PRESETS in lib/services/agent.service.ts
-- ============================================================================

-- --- OpenAI ---
UPDATE public.agents SET description =
  'Used by OpenAI to crawl web content that may be used to improve future AI models.'
WHERE name = 'GPTBot';

UPDATE public.agents SET description =
  'Used by OpenAI search features to discover and index publicly available web content.'
WHERE name = 'OAI-SearchBot';

UPDATE public.agents SET description =
  'Used when ChatGPT visits a URL on behalf of a user action, such as browsing or retrieval.'
WHERE name = 'ChatGPT-User';

UPDATE public.agents SET description =
  'Used by OpenAI agents to fetch and process web pages in response to user requests.'
WHERE name = 'OAI-SearchAgent';

-- --- Anthropic ---
UPDATE public.agents SET description =
  'Used by Anthropic to crawl web content for Claude-related indexing and model quality workflows.'
WHERE name = 'ClaudeBot';

UPDATE public.agents SET description =
  'Used for Claude web search discovery and retrieval of relevant public pages.'
WHERE name = 'Claude-SearchBot';

UPDATE public.agents SET description =
  'Used when Claude accesses a URL as part of an explicit user-initiated action.'
WHERE name = 'Claude-User';

UPDATE public.agents SET description =
  'General Anthropic AI crawler identity used for service-level automated retrieval tasks.'
WHERE name = 'anthropic-ai';

-- --- Google ---
UPDATE public.agents SET description =
  'Google''s main crawler used for Search indexing and serving search results.'
WHERE name = 'Googlebot';

UPDATE public.agents SET description =
  'Google product token publishers can use to manage whether site content is used for Gemini and Vertex AI generative features.'
WHERE name = 'Google-Extended';

UPDATE public.agents SET description =
  'General-purpose Google crawler used for product improvements, research, and quality checks outside core Search crawling.'
WHERE name = 'GoogleOther';

UPDATE public.agents SET description =
  'Used by Gemini deep research features to retrieve and analyze web content for user queries.'
WHERE name = 'Gemini-Deep-Research';

UPDATE public.agents SET description =
  'Used by Google Cloud Vertex AI services to access web resources for configured enterprise AI workflows.'
WHERE name = 'Google-CloudVertexBot';

-- --- Microsoft ---
UPDATE public.agents SET description =
  'Microsoft Bing''s primary crawler used to index web pages for Bing Search.'
WHERE name = 'bingbot';

UPDATE public.agents SET description =
  'Used by Bing to fetch page previews, snapshots, and rendering data for search experiences.'
WHERE name = 'BingPreview';

UPDATE public.agents SET description =
  'Microsoft advertising crawler used to review landing pages and ad quality signals.'
WHERE name = 'AdIdxBot';

-- --- Perplexity ---
UPDATE public.agents SET description =
  'Perplexity crawler used to discover and index public web content for answer generation.'
WHERE name = 'PerplexityBot';

UPDATE public.agents SET description =
  'Used when Perplexity accesses a page in direct response to a user request.'
WHERE name = 'Perplexity-User';

UPDATE public.agents SET description =
  'Used by Perplexity web search systems to retrieve sources for real-time answers.'
WHERE name = 'Perplexity-Search';

-- --- Amazon ---
UPDATE public.agents SET description =
  'Amazon crawler used to process web content for Amazon search and AI-powered services.'
WHERE name = 'Amazonbot';

-- --- Apple ---
UPDATE public.agents SET description =
  'Apple''s web crawler used by Siri and Spotlight suggestions and search-related features.'
WHERE name = 'Applebot';

UPDATE public.agents SET description =
  'Extended Apple crawler token for additional Apple Intelligence and generative feature usage controls.'
WHERE name = 'Applebot-Extended';

-- --- Meta ---
UPDATE public.agents SET description =
  'Meta external retrieval agent used to access public web content for AI and assistant features.'
WHERE name = 'Meta-ExternalAgent';

-- --- Mistral ---
UPDATE public.agents SET description =
  'Used when Mistral AI services access URLs on behalf of explicit user requests.'
WHERE name = 'MistralAI-User';

UPDATE public.agents SET description =
  'Mistral AI crawler used for web search, retrieval, and source discovery.'
WHERE name = 'MistralAI-SearchBot';
